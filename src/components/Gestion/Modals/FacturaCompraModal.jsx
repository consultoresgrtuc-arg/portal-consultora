...
const FacturaCompraModal = ({ isOpen, onClose, proveedorId, proveedorNombre, movimiento }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditMode) {
                await updateDoc(doc(db, 'users', user.uid, 'facturasCompra', movimiento.id), dataToSave);
            } else {
                await addDoc(collection(db, 'users', user.uid, 'facturasCompra'), dataToSave);
            }
            onClose();
        } catch (err) {
            console.error(err);
            setMessage('Error al guardar la factura de compra.');
            setIsSubmitting(false);
        }
    };
                ...
                <div className="flex justify-between items-start mb-8">
                    <div>
                        <h3 className="text-2xl font-black text-gray-900 tracking-tight">
                            {isEditMode ? 'Modificar Compra' : 'Nueva Factura de Compra'}
                        </h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <input 
                            ...
                            className="w-full px-5 py-4 bg-gray-50 border-transparent focus:bg-white focus:ring-4 focus:ring-orange-100 rounded-2xl transition-all outline-none font-bold text-gray-800"
                            placeholder="Ej: 0001-0000..."
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            ...
                            <input 
                                ...
                                required 
                                className="w-full px-12 py-4 bg-gray-50 border-transparent focus:bg-white focus:ring-4 focus:ring-orange-100 rounded-2xl transition-all outline-none font-black text-2xl text-gray-800"
                                placeholder="0.00"
                                step="0.01"
                            />
                        </div>
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        <button 
                            ...
                            className="flex-[2] bg-orange-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-orange-100 hover:bg-orange-700 transition-all transform active:scale-95 disabled:opacity-50"
                        >
                            {isSubmitting ? 'Guardando...' : (isEditMode ? 'Guardar Cambios' : 'Registrar Compra')}
                        </button>
                    </div>
                </form>
            </div>
