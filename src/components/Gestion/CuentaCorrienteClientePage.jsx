...
import FacturaVentaModal from './Modals/FacturaVentaModal';
import PagoRecibidoModal from './Modals/PagoRecibidoModal';

const CuentaCorrienteClientePage = ({ navigate, terceroId }) => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(docRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                setError(`No se encontró el cliente con ID: ${terceroId}`);
            }
        ...
        <div className="p-6 space-y-8 animate-fade-in">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex items-center gap-4">
                    <button 
                        onClick={() => navigate('gestion/deudores')} 
                    ...
                    <div>
                        <h2 className="text-3xl font-extrabold text-gray-800 tracking-tight">Cuenta Corriente</h2>
                ...
                <div className="flex gap-3">
                    ...
                    >
                        <Icon name="FilePlus" className="w-5 h-5 mr-2"/> Cargar Venta
                    </button>
                    <button 
            ...
            <div className="bg-white p-8 rounded-[32px] shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                <div className="absolute right-0 top-0 w-64 h-64 bg-blue-50 opacity-20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
                ...
                <div className="flex gap-8 text-center md:text-right">
                    <div className="px-6 border-r border-gray-100 last:border-0">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 italic">Ventas Totales</p>
                    ...
                    <table className="min-w-full divide-y divide-gray-50">
                        <thead className="bg-gray-50/50">
                            <tr>
                                ...
                                <th className="px-8 py-5 text-right text-[10px] font-black text-gray-400 uppercase tracking-widest">Debe (Fact)</th>
                                <th className="px-8 py-5 text-right text-[10px] font-black text-gray-400 uppercase tracking-widest">Haber (Pago)</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-50">
                            {loading ? (
                                <tr><td colSpan="6" className="text-center py-24 text-gray-400 font-bold animate-pulse">Recomponiendo historial de movimientos...</td></tr>
                                ...
                                <tr key={mov.id} className="hover:bg-gray-50/50 transition-colors group">
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="text-sm font-black text-gray-900">
                                            {mov.tipoMovimiento === 'factura' ? `Venta N° ${mov.numeroComprobante || 'S/N'}` : `Cobro Recibido`}
                                        </div>
                                        <div className="text-[10px] text-gray-400 font-medium mt-0.5">
                                            {mov.tipoMovimiento === 'factura' ? 'Comprobante de Venta' : `Medio: ${mov.medioDePago || 'Efectivo'}`}
                                        </div>
                                    </td>
            ...
            <FacturaVentaModal
                ...
                clienteId={terceroId}
                clienteNombre={terceroData?.nombre}
...
export default CuentaCorrienteClientePage;
